...
* Launch script, pass to it which pool and mode it should use, like `./primewatch.sh gpool stay`. There are two modes: "stay" and "jump". When stay (default) is used, script doesn't change the pool on connection hang. When jump is used, script will jump from one pool to another on hangs, constantly mining to the same DTC address. Currently xpool has 0.5 DTC payout barrier, while gpool 0.3 DTC. You can just leave the terminal on, or use `screen`, or put script in autostart, or do something else. I prefer the second option.
